...
void PedestalsAll(uint64_t gth, uint64_t Clock=10000, int points=100)
{
char nm[128];
Initi(Clock);
...
// c2->SetGridy(1);
// c2->Draw();
  sprintf(nm,"ASICs, Fx2=%d kHz, pedestals",Clock);
...
for(int chipid=0;chipid<256;chipid++)
{
  if(p->chipchannel[chipid]<0) continue;
//  hp->Reset(); 
...
{
  if(p->chipchannel[chipid]<0) continue;
 p->ASIC_threshold_global(chipid,50);
p->ASIC_channel_mask(chipid,0);
p->ASIC_external_trigger_mask(chipid,0);
 }
}
...
//       hp->Fill(pix,adc);
 ...
 // evcount=evcount+recvd/8;
  cycles++;
  if(cycles>1000000 && recvd==0){stopped=1; p->ASIC_channel_mask(chipid,0xffffffff); } // timeout
